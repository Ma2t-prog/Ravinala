...
class AdminPanel:
    ...
    def _render_dashboard(self, auth: AuthManager):
        ...
        if expiring_soon:
            st.markdown("---")
            st.warning(f"**{len(expiring_soon)} account(s) expiring within 7 days:**")
    ...
    def _render_security(self, auth: AuthManager, current_user: dict):
        ...
        st.warning("""
        **Nuclear Options** — These actions affect ALL users immediately.
        """)

        col1, col2 = st.columns(2)
        with col1:
            if st.button("Logout ALL users", width="stretch",
    ...
    def _render_quick_invite(self, auth: AuthManager):
        ...
        with st.form("quick_invite_form", clear_on_submit=False):
            ...
            submitted = st.form_submit_button("Generate Account", width="stretch")
        ...
        if invite:
            ...
            if st.button("Clear credentials", key="clear_invite"):
