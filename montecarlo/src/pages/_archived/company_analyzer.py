...
# ── top controls ───────────────────────────────────────────────────────────
_render_page_header("CA", "Company Analyzer", "Company intelligence, diagnostics and valuation overview", "Analyzer")
    ...
    with fs1:
        ...
        if not is_df.empty:
            ...
            if chart_rows:
                ...
                if fig_is:
                    st.plotly_chart(fig_is, width="stretch")
        else:
...
with tab_hr:
    ...
    if z_score is not None:
        ...
        fig_z.update_layout(paper_bgcolor="#0A0A0F", font=dict(color="#9ca3af"),
    ...
    try:
        ...
        f_details = {
            "ROA > 0 (Profitability)":          ("PASS" if p1 else "FAIL"),
            "Operating CF > 0":                 ("PASS" if p2 else "FAIL"),
            "ROA Improving YoY":                ("PASS" if p3 else "FAIL"),
            "Leverage Ratio Decreasing":        ("PASS" if p4_val else "FAIL"),
            "Current Ratio Improving":          ("PASS" if p5 else "FAIL"),
            "No New Shares Issued":             ("PASS" if p6 else "FAIL"),
            "Gross Margin Improving":           ("PASS" if p7 else "FAIL"),
            "Asset Turnover Improving":         ("PASS" if p8 else "FAIL"),
            "CF > Net Income (Low Accruals)":   ("PASS" if p9 else "FAIL"),
        }
    except Exception as e:
...
with tab_own:
    ...
    if inst_holders is not None and not inst_holders.empty:
        try:
            ...
            st.dataframe(ih_top, width="stretch")
    ...
    if insider_tx is not None and not insider_tx.empty:
        try:
            ...
            st.dataframe(it.reset_index(drop=True), width="stretch")
        except Exception as e:
