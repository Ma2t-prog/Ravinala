...
def _render_deal_blotter(bm: BookManager, book: Book, book_id: str):
    ...
    with h_col2:
        if st.button("🔄 Reprice All", use_container_width=True, key="reprice_all_btn"):
...
def _render_book_analytics(bm: BookManager, book_id: str):
    ...
    if HAS_REPORTING:
        ...
        with br1:
            if st.button("📋 Risk Report", use_container_width=True, key="btn_risk_rpt"):
        ...
        with br2:
            if st.button("💰 P&L Report", use_container_width=True, key="btn_pnl_rpt"):
        ...
        with br3:
            if st.button("📊 Export Excel", use_container_width=True, key="btn_xl_rpt"):
...
def _render_pnl_tracker(bm: BookManager, book_id: str):
    ...
    with col2:
        if st.button("📸 Take Snapshot", use_container_width=True, key="pnl_snap"):
...
def _render_trade_inspector(bm: BookManager, book: Book, book_id: str):
    ...
    with a1:
        if st.button("🔄 Reprice", key="insp_reprice", use_container_width=True):
            with st.spinner("Repricing..."):
                try:
                    ...
                    st.error(str(e))
    with a2:
        if st.button("📋 Clone", key="insp_clone", use_container_width=True):
    ...
    with a3:
        if st.button("💾 Template", key="insp_tpl", use_container_width=True):
            st.session_state['save_as_tpl_id'] = trade_id
    with a4:
        if t.status != TradeStatus.CANCELLED.value:
            if st.button("🗑️ Delete", key="insp_del", use_container_width=True):
                st.session_state['confirm_del_id'] = trade_id
    with a5:
        if st.button("📄 Reports ▾", key="insp_reports", use_container_width=True):
    ...
    if st.session_state.get('show_report_menu') == trade_id:
        if not HAS_REPORTING:
            ...
            with rc2:
                if st.button("📄 Term Sheet", key=f"btn_ts_{trade_id}", use_container_width=True):
            ...
            with rc3:
                if st.button("📊 Pre-Trade Report", key=f"btn_pt_{trade_id}", use_container_width=True):
            ...
            with rc4:
                ...
                if st.button("🎯 Client Deck", key=f"btn_cp_{trade_id}", use_container_width=True):
    ...
    if st.session_state.get('confirm_del_id') == trade_id:
        st.warning(f"⚠️ Soft-delete trade **{t.internal_ref}**? This sets status to CANCELLED.")
        dc1, dc2 = st.columns(2)
        with dc1:
...
| Field | Value |
|-------|-------|
| **Model** | {pr.model.replace('_', ' ').title()} {'⚠️ STALE' if pr.is_stale else ''} |
...
- **Barrier:** {c.get('condition_barrier_pct', '—')}%
    ...
    with st.expander("✏️ Edit Notes & Tags"):
...
def _render_new_trade(bm: BookManager, book_id: str):
    ...
    with st.form("new_trade_form", clear_on_submit=False):
        ...
        submitted = st.form_submit_button(
            "💰 Price & Save to Book", use_container_width=True, type="primary"
        )
    ...
    for tpl in templates:
        with st.expander(
            ...
            with col2:
                ...
                if st.button("🗑️ Delete", key=f"del_tpl_{tpl['file']}"):
...
def _render_book_settings(bm: BookManager):
    ...
    with st.expander("⚠️ Danger Zone"):
        ...
        if current_id != 'default':
            if st.button(f"🗑️ Delete '{current_name}'", key="del_book"):
